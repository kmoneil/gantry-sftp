...
python examples/concurrent_transfers.py           # a list, a fan-out and a tree, over one session
...
| `blocking.py`             | `gantry_sftp.sync`: a `with` instead of an `async with`, `walk` / `glob` as ordinary iterators, `scandir` still a context manager because it still holds a handle, a typed error crossing the thread flat, `BoundPortal` for several sessions on one loop or a backend other than asyncio — and `with_reconnect`, which runs your function on a third thread because the portal's own cannot use the session it is handed |
...
| `allowed_hosts.py`        | `allowed_hosts()` and `GANTRY_SFTP_ALLOWED_HOSTS`: restricting where a connection may go when the hostname came from somebody else — why layers only narrow, why the *effective* host is what is checked rather than the name you passed, the three things it deliberately does not do, and the `ControlPath` it refuses because a master already open to another host would carry the session there |
...
| `retry.py`                | `with_reconnect()`, `is_retryable()`, why a failed authentication is never retried, and the note that says whether it ran out of attempts or stopped early on one no reconnect could fix |
| `concurrent_transfers.py` | the three fan-out shapes and how to pick one: many `get()`s over one session with measured overlap and where an error lands once you fan out; what several failures at once report, since only one of them is raised and the rest arrive on a note; `get_many` / `put_many` for a list into one directory, with the derived destination name and the refusal when two sources flatten onto it; and `get_tree(concurrency=)`, a bounded pool rather than the same thing with a loop, plus `resume=` over a finished tree |
...
| `compatibility.py`        | `compatibility_report()` as **data before it is text**: the battery `python -m gantry_sftp doctor` prints, reached as objects a deployment check can assert on, with the verdict, the evidence behind each one, and the advertisement a server contradicts |
| `crash_resume.py`         | an upload the process does not survive: it forks a child that `SIGKILL`s itself mid-transfer, so no `finally` and no shielded cleanup run, and the parent finishes it from the journal — the staging name nothing else could have found |
