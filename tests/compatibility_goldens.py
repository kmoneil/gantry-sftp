...
# ---------------------------------------------------------------------------
# What the battery *asked*, as opposed to what it reported (D-193).
# ---------------------------------------------------------------------------

SCRATCH = b"/incoming/scratch/gantry-probe-t0ken-"
"""Prefix every scratch path shares, spelled out rather than built with the code's own `_join`.

Deriving it would make this an expectation computed with the code under test: a mutation of
`_join` would move both sides together and the comparison would pass. Written as a literal, a
`_join` that changes moves the *actual* only, and the row fails.
"""

HOME = b"/home/probe/gantry-probe-t0ken-"
"""The other prefix, for the two probes that ask about a path outside the write directory."""

CREATE = OpenFlag.WRITE | OpenFlag.CREAT | OpenFlag.EXCL
"""The create-exclusively flag set `_create` uses. Named because it appears seventy times."""
EXCHANGES: dict[str, tuple[tuple[str, object], ...]] = {
    "capable": (
        ("realpath", b"."),
        ("realpath", HOME + b"absent"),
        ("realpath", b"/"),
        ("stat", HOME + b"absent"),
        ("open", (SCRATCH + b"case-aA", CREATE, 0o600)),
        ("close", b"handle"),
        ("stat", b"/incoming/scratch/GANTRY-PROBE-T0KEN-CASE-AA"),
        ("open", (SCRATCH + b"rename-source", CREATE, 0o600)),
        ("close", b"handle"),
        ("open", (SCRATCH + b"rename-target", CREATE, 0o600)),
        ("close", b"handle"),
        ("rename", (SCRATCH + b"rename-source", SCRATCH + b"rename-target")),
        ("open", (SCRATCH + b"times", CREATE, 0o600)),
        ("close", b"handle"),
        ("utime", (SCRATCH + b"times", 1000000000, 1000000000)),
        ("stat", SCRATCH + b"times"),
        ("open", (SCRATCH + b"posix-source", CREATE, 0o600)),
        ("close", b"handle"),
        ("open", (SCRATCH + b"posix-target", CREATE, 0o600)),
        ("close", b"handle"),
        ("posix_rename", (SCRATCH + b"posix-source", SCRATCH + b"posix-target")),
        ("exists", SCRATCH + b"posix-source"),
        ("open", (SCRATCH + b"fsync", CREATE, 0o600)),
        ("write_at", (0, 29)),
        ("fsync", b"handle"),
        ("close", b"handle"),
        ("open", (SCRATCH + b"lsetstat-target", CREATE, 0o600)),
        ("close", b"handle"),
        ("symlink", (SCRATCH + b"lsetstat-target", SCRATCH + b"lsetstat-link")),
        ("chmod", (SCRATCH + b"lsetstat-link", 0o640, False)),
        ("lstat", SCRATCH + b"lsetstat-link"),
        ("stat", SCRATCH + b"lsetstat-target"),
        ("open", (SCRATCH + b"check-file", CREATE, 0o600)),
        ("write_at", (0, 29)),
        ("close", b"handle"),
        ("open", (SCRATCH + b"check-file", OpenFlag.READ, None)),
        ("check_file", b"handle"),
        ("close", b"handle"),
        ("open", (SCRATCH + b"largest-request", CREATE, 0o600)),
        ("write_at", (0, 4096)),
        ("close", b"handle"),
        ("remove", SCRATCH + b"largest-request"),
        ("remove", SCRATCH + b"check-file"),
        ("remove", SCRATCH + b"lsetstat-link"),
        ("remove", SCRATCH + b"lsetstat-target"),
        ("remove", SCRATCH + b"fsync"),
        ("remove", SCRATCH + b"posix-target"),
        ("remove", SCRATCH + b"posix-source"),
        ("remove", SCRATCH + b"times"),
        ("remove", SCRATCH + b"rename-target"),
        ("remove", SCRATCH + b"rename-source"),
        ("remove", SCRATCH + b"case-aA"),
    ),
    "informative": (
        ("realpath", b"."),
        ("realpath", HOME + b"absent"),
...
"""Every call each portrait's battery makes, in order, with its arguments.

**Why this exists alongside `REPORTS`, which already pins each report whole.** A report golden
can only see what a probe *said*. `StubSession` scripts its answers independently of the
arguments it is handed -- that is what makes it a stub -- so a probe that asks the wrong
question still gets the scripted answer and still emits the identical finding. Measured, not
argued: of 98 surviving mutants in `compatibility.py`, **46 replace an argument with `None` or
change a keyword**, and no number of additional portraits can reach one of them.

The case that names the cost is `_probe_lsetstat`. It exists solely to ask whether the server
honours `follow_symlinks=False`, and three of its five survivors are that argument turning into
`True`, into `None`, and into nothing at all -- the last defaulting to `True`, so the probe
measures the opposite of its own purpose and reports a confident verdict about it. That verdict
is what a reader uses to decide whether atomic publish is safe on their endpoint.

**Seven entries, four distinct sequences.** `restating`, `informative` and `silent` vary only in
*how* they refuse, which changes the report and not the questions, so their exchanges are
byte-identical to each other. Kept per portrait anyway, so this pairs with `REPORTS` and
`SERVERS` by name and a new portrait cannot be added here without a stub; the redundancy is
asserted rather than left to be noticed, by `test_the_exchanges_are_less_varied_than_the_reports`.

**Regenerating**, on the same terms as `REPORTS` and for the same reason -- these come from the
code under test, so they pin behaviour and cannot prove it. Read each call against the probe that
makes it and ask whether that is the question the docstring says it asks, because a golden pasted
without that step freezes a probe that was already asking the wrong thing.
"""
