...
# What the local filesystem will actually hold -- the non-UTF-8 probe, its marker, and the
# case-folding helper -- moved to `local_filesystem.py`, a plain module beside this one.
# `live-tests/` has a conftest of its own, so `from conftest import ...` cannot reach here from
# there, and the live fsspec lane consequently asked the filesystem nothing: nine rows errored
# at setup on macOS while their unit twins skipped. Same shape, and same fix, as
# `server_contract.py` (D-114) -- a rule answered once, reachable from every lane.
